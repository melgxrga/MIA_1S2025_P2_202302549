...
func ReadFile(tablaInodo *datos.TablaInodo, path string, superbloque *datos.SuperBloque) string {
	...
	fmt.Println("Depuración: Contenido de users.txt:")
...
func FindAndCreateDirectories(tablaInodo *datos.TablaInodo, path, ruta string, superbloque *datos.SuperBloque, posicionActual, userId, groupId int64) {
	fmt.Printf("🔍 Depuración: Intentando crear directorios en ruta -> %s, Posición Actual: %d\n", ruta, posicionActual)

	if !strings.Contains(ruta, "/") {
		fmt.Println("✅ Depuración: Llegamos al último nivel, no hay más directorios que crear.")
		return
	}

	rutaParts := strings.SplitN(ruta, "/", 2)
	fmt.Printf("📂 Depuración: Siguiente directorio a buscar/crear -> %s\n", rutaParts[0])

	for i := 0; i < len(tablaInodo.I_block); i++ {
		var bloqueCarpeta datos.BloqueDeCarpetas

		if tablaInodo.I_block[i] == -1 {
			// Creando nuevo bloque de carpeta
			fmt.Println("🆕 Depuración: Creando un nuevo bloque de carpeta...")
			LlenarBloqueCarpetaVacio(&bloqueCarpeta)

			posicionNuevaBloque := bitmap.WriteInBitmapBlock(path, superbloque)
			if posicionNuevaBloque == -1 {
				fmt.Println("❌ Error: No se pudo asignar un nuevo bloque en el bitmap.")
				return
			}
			fmt.Printf("✅ Depuración: Nuevo bloque asignado en posición %d\n", posicionNuevaBloque)
			...
			if posicionNuevaTablaInodo == -1 {
				fmt.Println("❌ Error: No se pudo asignar un nuevo inodo en el bitmap.")
				return
			}
			fmt.Printf("✅ Depuración: Nuevo inodo asignado en posición %d\n", posicionNuevaTablaInodo)
			...
			fmt.Printf("✅ Depuración: Directorio '%s' creado con éxito en inodo %d\n", rutaParts[0], posicionNuevaTablaInodo)
		...
		if compare {
			fmt.Printf("✅ Depuración: Directorio '%s' ya existe en inodo %d\n", rutaParts[0], num)
		...
		if FreeSpace(&bloqueCarpeta) {
			fmt.Println("🆕 Depuración: Hay espacio en un bloque existente, creando nuevo directorio...")
			...
			fmt.Printf("✅ Depuración: Directorio '%s' agregado en inodo %d dentro de un bloque existente\n", rutaParts[0], posicionNueva)
